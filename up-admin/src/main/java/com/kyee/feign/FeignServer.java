...
public class FeignServer {
    ...
    public Map<String, Object> updateState(@RequestBody HashMap paramMap) {
        ...
        if ("Y".equals(mergePackageFlag)) {
            List<Long> patchIdList = upPatchMapper.selectSubUpPatchIdListByPatchFileName(patchFileName);
    ...
    public Map<String, Object> pullValidatePatch(@RequestBody HashMap paramMap) {
        ...
        if (!Objects.isNull(pp) && ConstantUtil.PatchBranch.COMMITID.getValue().equals(pp.getPatchBranch())) {
            ...
            if (!Objects.isNull(patch)) {
                tip = validatePatchMergeTime(patch);
            }
        }
        Map<String, Object> resultMap = new HashMap<>();
        resultMap.put("code", 0);
     ...
     */
    private String validatePatchMergeTime(UpPatch upPatch) {
        // 已发布的包列表
        List<UpPatch> patchList = upPatchMapper.getPatchListByStatus(upPatch, ConstantUtil.UpPatchStatus.PUBLISHED.getValue());
        if (!CollectionUtils.isEmpty(patchList)) {
            if (CollectionUtils.isEmpty(patchList)) {
                return "";
            }
