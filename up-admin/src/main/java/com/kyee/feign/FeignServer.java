...
public class FeignServer {
    ...
    public Map<String, Object> getUpPatchByPatchId(@RequestBody HashMap paramMap) {
        ...
        if (!CollectionUtils.isEmpty(upPatchCodelists)) {
            ...
            if (!CollectionUtils.isEmpty(compileList)) {
                ...
                // 将HIS版本号路径code/his/version.xml添加到编译路径中,供一键升级
                if (compiles.contains("code/his/")) {
                    compiles += ",code/his/version.xml";
                }
            }
        }
     ...
     * @return
     */
    @PostMapping("/pullValidatePatch")
    public Map<String, Object> pullValidatePatch(@RequestBody HashMap paramMap) {

        boolean isCommitId = false;
        String tip = "";
        Integer productId = Integer.valueOf(paramMap.get("productId") + "");
        Integer projectId = Integer.valueOf(paramMap.get("projectId") + "");
        Long patchId = Long.valueOf(paramMap.get("patchId") + "");
        String pulledStr = paramMap.get("pulledList") + "";
        log.info("已经拉取的patchId: " + pulledStr);
        List<String> pulledList = Arrays.asList(pulledStr.split(","));

        UpProjectProduct pp = projectProductMapper.getUpProjectProductById(productId, projectId);
        if (!Objects.isNull(pp) && ConstantUtil.PatchBranch.COMMITID.getValue().equals(pp.getPatchBranch())) {
            // 判断当前包和已发布的包有没有重复代码列表 YUNHIS-37875
            UpPatch patch = upPatchMapper.selectUpPatchById(patchId);
            if (!Objects.isNull(patch)) {
                tip = validatePatchMergeTime(patch, pulledList);
            }
        }
        Map<String, Object> resultMap = new HashMap<>();
        resultMap.put("code", 0);
     ...
     */
    private String validatePatchMergeTime(UpPatch upPatch, List<String> pulledList) {
        // 已发布的包列表
        List<UpPatch> patchList = upPatchMapper.getPatchListByStatus(upPatch, ConstantUtil.UpPatchStatus.PUBLISHED.getValue(), pulledList);
        if (!CollectionUtils.isEmpty(patchList)) {
            if (CollectionUtils.isEmpty(patchList)) {
                return "";
            }
            ...
            List<UpPatchCodeListPo> codeList = patchCodelistMapper.getCodePathWithIds(ids, productId, projectId);
            codeList.forEach(s -> s.setCodePath(s.getCodePath().replaceAll("\r|\n", "")));

            List<UpPatchCodelist> codeLists = patchCodelistMapper.selectCodeListByPatchId(patchId);
            ...
            if (!CollectionUtils.isEmpty(codeList)) {
                for (UpPatchCodeListPo codeListPo : codeList) {
                    if (codeListPublish.contains(codeListPo.getCodePath())) {
                        ...
                        fileNameList.add(patchFileName);
                    }
                }
            }
            if (!CollectionUtils.isEmpty(fileNameList)) {
                // 给出提示
                return "请注意！系统检测到存在编译时间小于当前包的补丁包【" + Arrays.toString(fileNameList.toArray()) +
                        "】，且与当前包的文件列表有重复。请先拉取补丁包或合并补丁包，否则将引发跨包升级问题！";
            }
        }
        return "";
    }
