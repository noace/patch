...
public class HisV3PatchServiceImpl implements IPatchService {
    ...
    private UpPatch createPath(String sourceUrl,String processId
        ...
        for(UpPatchCodelist tempEntity: upPatchCodelistResult){
            ...
            if (!"Y".equals(mergePackageFlag)) {
                ...
                if (fileName.contains("_TODO.SQL") && fileName.contains("ALTER")) {
                    todoSqlFlag = true;
                    LogUtil.recordBuildLog("--------------校验TODO脚本开始: " + fileName + "--------------", upPatch.getPatchId(), processId);
                    String tableName = fileName.substring(fileName.lastIndexOf("_ALTER_") + 7, fileName.lastIndexOf("_TODO.SQL"));
    ...
    public List<String> commitIdCopySourceToTempDir(String sourceUrl,String tempSourcePath,Map<Long, String> codeMap, UpPatch upPatch, String processId, String mergePackageFlag) throws Exception {
        ...
        // Map Key排序
        Map<Long, String> treeMap = new TreeMap<>(codeMap);
        log.info("------------------------------------MAP:" + treeMap.keySet() + "------------------------------------");
        for (Long codeId : treeMap.keySet()) {
            boolean jar = false;
            ...
            if (!DotNetToJavaStringHelper.isNullOrEmpty(tepStr)) {
                ...
                if (tepStr.contains("src/main/webapp/") || (tepStr.contains("src/main/resources/") && !tepStr.contains(".xml"))) {
                    jar = true;
                ...
                if (tepStr.toLowerCase().endsWith(".java")) {
                    ...
                    String sqlCompilePath = copySqlFile(sourceUrl, tempSourcePath, sourcePath, localsubPath,
                                                                    patchId,
                                                                    processId,
                                                                    mergePackageFlag,
                                                                    createBy);
        ...
        if(sourceFile.exists()){
            ...
            if (!"Y".equals(mergePackageFlag)) {
                ...
                if(!sourceFilePath.contains(path)) {
                    ...
                    executeCommonSQL.verifyParam(sourceFilePath, patchId, processId);
                } else {
