...
//    @Scheduled(cron = "0 30 17 ? * *")
    ...
    // 0 0/30 9-18 * * ?    每天9点到18点
//    @Scheduled(cron = "0 0 9,11,14,16,18 * * ?")
...
//        int count = messageService.getPatchWithSuccess(deptId);
        ...
        // 统计当天未发布的补丁包
        List<Map<String, String>> nameMap = messageService.getCountByUsername(deptId);
        ...
        if (nameMap.size() != 0) {
            ...
            if (!CollectionUtils.isEmpty(nameMap)) {
                for (Map<String, String> map : nameMap) {
                    ...
                    sb.append(">").append(map.get("name")).append(":<font color=\"comment\">").append(total).append("</font>\n");
                    count += total;
                }
            }
            ...
            textText.setMentioned_list(userIds.stream().distinct().collect(Collectors.toList()));
    ...
    private void failMessageService(String key, String deptId) {
        ...
        MessageText textText = new MessageText();
        StringBuilder sb = new StringBuilder();
        // 查询打包失败的补丁包(LIS项目单独统计)
        Integer productId = 7;
        List<Map<String, String>> failMap = messageService.getPatchWithFailAndBug(deptId, productId);
