...
public class HisV3FullPackService {
    ...
    public UpPatch fullPack(String sourceUrl, String branch, String processId, Long patchId, String repository) throws Throwable {
        ...
        // 校验源码中是否有重复路径的文件
        // verifySourceCodeRepeat(sourceUrl, patchId, processId, projectProduct);
        ...
        UpPatch tempUpPatch;
        if (ConstantUtil.BuildType.CLASS_BUILD.getValue().equals(projectProduct.getBuildType())) {
    ...
    public UpPatch executePkg(String sourceUrl,Long patchId, String processId, UpProjectProduct projectProduct) throws Exception {
        ...
        String path = strTempPath + "code" + File.separator + shortName + File.separator;
        clearAndCreateTempDir(path);

        // 复制全量包文件到临时目录
        org.aspectj.util.FileUtil.copyFile(new File(packPath), new File(path));
    ...
    public UpPatch executePkgClass(String sourceUrl,Long patchId, String processId, UpProjectProduct projectProduct) throws Exception {
        ...
        String dirPath = strTempPath + "code" + File.separator + shortName + File.separator;
        clearAndCreateTempDir(dirPath);
        // 复制全量包文件到临时目录
        org.aspectj.util.FileUtil.copyFile(new File(packPath), new File(dirPath));
...
//            List<String> fileList = new ArrayList<>();
        ...
        LogUtil.recordBuildLog("--------------校验Jar重复文件总耗时：" + (endMillis-startMillis) + "ms--------------", patchId, processId);
     ...
     * @date：2023-09-15
     */
    private void verifySourceCodeRepeat (String sourceUrl, Long patchId, String processId, UpProjectProduct projectProduct) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(upProjectModules)) {
            ...
            if (repeatPath.size() > 0) {
                throw new Exception("项目源码文件路径有冲突！请处理之后，再生成完整包！冲突文件，详见日志！！！");
            } else {
     ...
     * @throws Exception
     */
    private static List<Map<String, Long>> recursionSourceFileDir(File dir) {
        List<Map<String, Long>> mapList = new ArrayList<>();
