...
public class UpPatchClientServiceImpl implements IUpPatchClientService
{
    ...
    @Autowired
    private UpProjectServerMapper projectServerMapper;
    
    @Autowired
    private UpPatchCodelistMapper codelistMapper;
...
//        }
    ...
    public AjaxResult tempUpgrade(Long patchId, String patchFileName, String mergePackageFlag, String isUpgrade) {
        ...
        upPatchClient.setPatchId(patchId);
        upPatchClient.setPatchStatus(ConstantUtil.UpgradeStatus.UPGRADE_ALL.getValue());
        // 修改时间，上传人， 状态 确定升级信息
        upPatchClient.setUpdateTime(new Date());
        upPatchClient.setUpBy(userName);
        if ("Y".equals(mergePackageFlag)) {
            upPatchClient.setMergePackageFlag(String.valueOf(patchId));
        }
        upPatchClientMapper.updatePatchAndSubPatchById(upPatchClient);

        // 推送云补丁包状态
        HashMap<String, Object> paramMap = new HashMap<>();
        paramMap.put("patchFileName", patchFileName);
        paramMap.put("patchStatus", ConstantUtil.UpgradeStatus.UPGRADE_ALL.getValue());
    ...
    public AjaxResult upgradePatch(Long patchId, String patchFileName, String mergePackageFlag, String isUpgrade, String patchFileUrl) throws Exception {
        // 获取到补丁管理服务器的补丁包解压后放到远程服务器目录下
        // String[] urls = patchFileUrl.split("HIS_V3");

        String profile = RuoYiConfig.getProfile(); // profile: /opt/kyup/uploadPath
        String zipback = patchFileName.split(".ZIP")[0];
        System.out.println(zipback);
        String unzipUrl = profile + "/HIS_V3/"+zipback+"/";
        System.out.println(unzipUrl);
        String url = profile + "/HIS_V3/" + patchFileName;

        // String unzipUrl = urls[0] + "unzip" + "/";
        // 解压包
        System.out.println("解压开始");
        boolean unzipFlag = ZipFileUtil.unZipFiles(new File(url), unzipUrl);
        System.out.println("解压结束");
        if (unzipFlag) {
            // 解压成功，复制文件到目标服务器
            // 测试服务器
            UpProjectServer server = new UpProjectServer();
            server.setProductId(1);
            server.setProjectId(1);
            List<UpProjectServer> servers = projectServerMapper.selectUpProjectServerList(server);
            if (!CollectionUtils.isEmpty(servers)) {
                UpProjectServer projectServer = servers.get(0);
                System.out.println(projectServer);
                String serverIp = projectServer.getServerIp();
                String sshUser = projectServer.getSshUser();
                String sshPassword = projectServer.getSshPassword();
                Integer sshPort = projectServer.getSshPort();
                // 目标服务器代码存放路径
                String serverPath = projectServer.getServerPath();
                System.out.println(unzipUrl);
                
                /*String cmd = "scp -r "+ unzipUrl + "code/his/ "+ sshUser + "@" + serverIp + ":/opt/test/his/";
                System.out.println(cmd);
                System.out.println("sshUser:"+sshUser+",sshPassword:"+sshPassword+",serverIp:"+serverIp+",sshPort:"+sshPort);
                RemoteCommandUtil.remoteCommandUtil(sshUser, sshPassword, serverIp, sshPort, cmd);*/
                boolean usePassword = true;
                // 获取补丁包解压后的文件路径
                /*UpPatchCodelist codelist = new UpPatchCodelist();
                codelist.setPatchId(patchId);*/
//                List<UpPatchCodelist> upPatchCodelists = codelistMapper.selectUpPatchCodelistList(codelist);
                UpPatchClient patchClient = clientMapper.selectUpPatchClientById(patchId);
                if (!Objects.isNull(patchClient)) {
                    // 集合转数组
//                    String[] files = (String[])upPatchCodelists.stream().map(UpPatchCodelist::getCodePath).toArray();
//                  String[] array1=(String[])codePathList.toArray(new String[codePathList.size()]);
                    String[] files = patchClient.getCodeList().split(",");
                    SCPClient scpClient = SCPFile.createSCPClient(serverIp, sshPort, sshUser, sshPassword, null, usePassword);
                    for (String file : files) {
                        System.out.println(file);
                        // 拼code
                        /*StringBuilder sb = new StringBuilder();
                        sb.append(unzipUrl).append("code/");*/
                        /*file = unzipUrl+"code/"+file;
                        System.out.println(file);*/
                        int start = file.indexOf('/') + 1;
                        int end = file.lastIndexOf('/') + 1;
                        String fileUrl = file.substring(start, end);
                        String file2 = unzipUrl + file;
//                        String file3 = Pattern.compile("\n").matcher(file2).replaceAll("");
                        System.out.println(file2);
                        if(!Objects.isNull(scpClient)) {
                            // 传输数组文件
                            scpClient.put(file2, serverPath+fileUrl);
                            System.out.println(serverPath+fileUrl);
                        }
                    }
                }
//                TestFTP.putFile(localFile, remoteFile, serverIp, sshPort, sshUser, sshPassword, null, usePassword);
                // String run = CommandUtil.run(cmd);
                // System.out.println(run);
                System.out.println("命令执行完毕");
            } else {
                throw new Exception("没有找到对应的服务器");
            }
     ...
     */
    @Override
    public AjaxResult downloadPatchFile(String patchId) {
