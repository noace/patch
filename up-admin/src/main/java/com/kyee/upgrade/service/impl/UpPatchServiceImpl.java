...
public class UpPatchServiceImpl implements IUpPatchService
{
    ...
    public String registPatch(UpPatch upPatch) throws Exception {
        ...
        if (StringUtils.isNotEmpty(upPatch.getDeletePath()) && !remark.contains("删除")) {
            throw new Exception("合并的代码中包含删除的文件，请检查备注删除哪些文件: \n " + upPatch.getDeletePath());
        }

        if (StringUtils.isNotEmpty(upPatch.getMovePath()) && !remark.contains("删除")) {
            throw new Exception("合并的代码中包含移动的文件，需检查备注删除哪些文件: \n " + upPatch.getMovePath());
        }
        ...
        if (ConstantUtil.PatchBranch.COMMITID.getValue().equals(pp.getPatchBranch()) && !"Y".equals(upPatch.getMergePackageFlag())) {
            String projectPrincipal = pp.getProjectPrincipal();
            List<String> projectPrincipals = Arrays.asList(projectPrincipal.split(","));
            validatePatchMergeTime(upPatch, projectPrincipals.get(0));
        }

        //生成信息到构建队列
        UpCommand upCommand = setUpCommandParam(upPatch);

        amqpTemplate.convertAndSend("hello", upCommand);

        log.info("rabbitMq 发送消息：" + upCommand.getPatchId());
...
//        upCommandMapper.insertUpCommand(upCommand);
     ...
     */
    private void validatePatchMergeTime(UpPatch upPatch, String projectPrincipal) throws Exception {

        // TODO upPatch 合并时间是否为空
        ...
        if (StringUtils.isEmpty(mergeTime)) {
            throw new Exception("补丁包合并时间为空，请检查！");
        }
        ...
        if (!CollectionUtils.isEmpty(patchList)) {
            ...
            commonGetCodeListByIds(publishPatchList, patchId, productId, projectId, projectPrincipal, "publish");
            commonGetCodeListByIds(upgradePatchList, patchId, productId, projectId, projectPrincipal, "upgrade");
        }
    }
     ...
     * @param type
     */
    private void commonGetCodeListByIds(List<UpPatch> patchList, Long patchId, Integer productId, Integer projectId, String projectPrincipal, String type) throws Exception {
        ...
        List<UpPatchCodelist> codeLists = upPatchCodelistMapper.selectCodeListByPatchId(patchId);
        codeLists.forEach(s -> s.setCodePath(s.getCodePath().replaceAll("\r|\n", "")));
        List<String> codeListPublish = codeLists.stream().distinct().filter(s -> !"SQL".equals(s.getFileType()))
                                                    .map(UpPatchCodelist::getCodePath).collect(Collectors.toList());
            ...
            for (UpPatchCodeListPo codeListPo : codeList) {
                if (codeListPublish.contains(codeListPo.getCodePath())) {
                    ...
                    if ("publish".equals(type)) {
                        String publishMsg = "【警告】由于您合并分支后未及时打包，系统检测到有其他补丁包携带你的部分代码【已发布至现场】！请立刻按照以下流程处理！\n" +
                                "1、立刻通知升级负责人【" + projectPrincipal + "】不要升级【" + patchFileName +"】;\n" +
                                "2、打包完成后请及时发布，并通知升级负责人【先升级您的补丁包再升级" + patchFileName + "】";
...
//                        tencentMessageJob.sendMessage(textMsg, hisKey);
                    ...
                    if ("upgrade".equals(type)) {
                        String upgradeMsg = "【警告】由于您合并分支后未及时打包，系统检测到有其他补丁包【" + patchFileName + "】已携带你的部分代码发布至现场，且【已经升级】！请立刻按照以下流程处理！\n" +
                        ...
                        textMsg.setText(textText);
//                        tencentMessageJob.sendMessage(textMsg, hisKey);
                        throw new Exception(upgradeMsg);
                    }
                }
            }
        }
