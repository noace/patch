...
public class UpPatchSqlServiceImpl implements IUpPatchSqlService
{
     ...
     */
    @Override
    public Map<String, String> viewSql(Long id) {
        Map<String,String> result = new HashMap<>();
        StringBuffer sql = new StringBuffer();
        UpPatchSql upPatchSql = upPatchSqlMapper.selectUpPatchSqlById(id);
        String path = upPatchSql.getSqlPath();
        String sqlName = path.substring(path.lastIndexOf("/") + 1);
        String sourcePath = RuoYiConfig.getProfile() + "/SQL/" + sqlName;
        InputStreamReader reader = null;
        BufferedReader inBuff = null;
        try {
            reader = new InputStreamReader(new FileInputStream(sourcePath), Charset.forName("GBK"));
            inBuff = new BufferedReader(reader);
            String line;
        ...
        result.put("sqltext",sql.toString());
        return result;
    }
}
