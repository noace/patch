...
//                    }
                ...
                log.info(e.getMessage());
                LogUtil.recordUpgradeLog("补丁包文件上传失败【" + patchFileName + "】" + e.getMessage(), patchId, serverId);
                return updateServerStatus(server, patchId, null, ConstantUtil.UpgradeRecordStatus.UPGRADE_FAIL.getValue(), "补丁包包文件上传失败:" + e.getMessage());
            }
...
//        String msg = syncCloudPatchStatus(patchClient);
     ...
     */
    @Override
    public List<UpUpgradeDataDTO> getDatabaseList(UpProjectServer server) throws ParseException {
        ...
        if (!CollectionUtils.isEmpty(databaseList)) {
            for (UpUpgradeDataDTO upgradeDataVo : databaseList) {
                ...
                Map<String, Object> unUpgradeMap = upProjectServerMapper.selectUnExecute(upgradeDataVo, ConstantUtil.UpgradeStatus.NOT_TEST.getValue(), ConstantUtil.UpgradeStatus.NOT_UPGRADE.getValue());
                upgradeDataVo.setNoUpNum(Integer.parseInt(unUpgradeMap.get("noUpNum") + ""));
                upgradeDataVo.setNoUpMinTime(dateFormat.parse(unUpgradeMap.get("noUpMinTime") + ""));
                //查询该服务器最后升级包、最后升级人员、最后升级时间
                UpUpgradeDataDTO patchDTO = new UpUpgradeDataDTO();
