...
(function ($) {
    $.extend({
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == ('' + value)) {
                    	...
                    	actions.push($.common.sprintf("<span class='%s' style='cursor:hand'>%s</span>", listClass, dict.dictLabel));
                        return false;
                    }
                });
                return actions.join('');
    	...
    	modal: {
            ...
            // 确认窗体
            confirmCustom: function (btn1,btn2,content, callBack) {
                layer.confirm(content, {
                    icon: 3,
                    title: "系统提示",
                    btn: [btn1, btn2]
            ...
            openOptions: function (options) {
                ...
                var _width = $.common.isEmpty(options.width) ? "850" : options.width;
        ...
        operate: {
            ...
            submit: function(url, type, dataType, data, callback) {
                var config = {
                    ...
                    success: function(result) {
                        if (result.code === 0 && typeof callback == "function") {
                            callback(result.msg);
                        }
                        // result.msg = "操作成功";
            ...
            canclePublish: function(id) {
                debugger;
                table.set();
                $.modal.confirm("确定取消发布该条" + table.options.modalName + "信息吗？", function() {
                    var url = $.common.isEmpty(id) ? table.options.canclePublishUrl : table.options.canclePublishUrl.replace("{id}", id);
                    if(table.options.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = { "ids": id };
            ...
            // 取消合并
            cancleMerge: function(id) {
                debugger;
                table.set();
                $.modal.confirm("是否确定取消合并该补丁包？", function() {
                    var url = $.common.isEmpty(id) ? table.options.cancleMergeUrl : table.options.cancleMergeUrl.replace("{id}", id);
                    if(table.options.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = { "id": id };
            ...
            downloadFile: function(resource) {
                ...
                window.location.href = ctx + "common/download/resource?resource=" + encodeURI(resource);
            },
            // 批量删除信息
            removeAll: function() {
            	table.set();
            ...
            download: function() {
                ...
                $.modal.confirm("确认要下载选中的" + rows.length + "条数据吗?", function() {
                    var url = table.options.downloadUrl;
                    var data = { "ids": rows.join() };
            ...
            //检查重复登记
            check: function(url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
            ...
            expedited: function() {
                table.set();
                $.modal.confirm("是否确定加急该补丁包？", function() {
                    var rows = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    var status = $.table.selectAllColumns('patchStatus');
                    var jiraNos = $.table.selectAllColumns('jiraNo');
                    var topics = $.table.selectAllColumns('topic');
                    var expediteds = $.table.selectAllColumns('expedited');
                    if (rows.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录！");
                        return;
                    }
                    ...
                    if (status && status.length > 0) {
                        for (var index = 0; index < status.length; index++) {
                            // 10 未升级 15:已发布 20 已打包
                            if (status[index] != '10' && status[index] != '15' && status[index] != '20') {
                                msgs.push("任务号【" + jiraNos[index] + "】【" + topics[index] + "】<br/>");
                            }
                            if (expediteds[index] == 'Y'){
                                msgs2.push("任务号【" + jiraNos[index] + "】【" + topics[index] + "】<br/>");
                            }
                        }
                    }
                    if (msgs.length != 0 || msgs2.length !=0) {
                        var message = '';
                        if (msgs.length != 0){
                            message = msgs.join("");
                            message += "不是已打包或未升级或已发布状态，不能加急，请重新选择！<br/>";
                        }
                        if (msgs2.length != 0){
                            var mssage2 = msgs2.join("");
                            message += mssage2 + "已经进行过加急，请重新选择！"

                        }
                        $.modal.alertWarning(message);
                        return;
                    }
                    var productNames = $.table.selectAllColumns('productName');
                    var projectName = $.table.selectAllColumns('projectName');
                    var developer = $.table.selectAllColumns('updateBy');
                    var patchFileName = $.table.selectAllColumns('patchFileName');
                    var projectIds = $.table.selectAllColumns('projectId');//项目
                    var productIds = $.table.selectAllColumns('productId');//产品
                    var url = table.options.setExpeditedUrl;
                    var data = {
                        "ids": rows.join(),
                        "status": "Y",
                        "upPatchStatus": status.join(),
                        "jiraNos": jiraNos.join(),
                        "topics": topics.join(),
                        "projectIds": projectIds.join(),
                        "productIds": productIds.join(),
                        "productNames": productNames.join(),
                        "projectNames": projectName.join(),
                        "developer": developer.join(),
                        "patchFileName": patchFileName.join()
                    };
                    $.operate.submit(url, "post", "json", data);
                });
            },
            cancelExpedited: function() {
                table.set();
                var rows = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一条记录！");
